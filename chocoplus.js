...
// --- NUEVO: Gestión de Tiempo de Inicio Persistente ---
const startTimeFile = path.join(__dirname, 'bot_start_time.json'); // Nombre del archivo de memoria
let initialStartTime; // Aquí guardaremos la hora del primer inicio (en segundos)

try {
  // Intenta leer el archivo de memoria
  if (fs.existsSync(startTimeFile)) {
    // Si existe, lee la hora guardada
    const data = JSON.parse(fs.readFileSync(startTimeFile, 'utf8'));
    initialStartTime = data.initialStartTime; // Usa la hora guardada
    console.log(`[UPTIME] Usando tiempo de inicio guardado: ${new Date(initialStartTime * 1000).toISOString()}`);
  } else {
    // Si NO existe, es la primera vez que arranca
    initialStartTime = Math.floor(Date.now() / 1000); // Guarda la hora actual
    // Crea el archivo de memoria con la hora actual
    fs.writeFileSync(startTimeFile, JSON.stringify({ initialStartTime }), 'utf8');
    console.log(`[UPTIME] Primer inicio detectado. Guardando tiempo: ${new Date(initialStartTime * 1000).toISOString()}`);
  }
} catch (error) {
  // Si algo falla al leer/escribir, usa la hora actual como emergencia
  console.error('[UPTIME] Error al leer/guardar tiempo de inicio. Usando tiempo actual.', error);
  initialStartTime = Math.floor(Date.now() / 1000);
}
// --- FIN NUEVO ---
...
async function isFullyConnected(sessionPath) {
  try {
    ...
    // 3. Tu lógica original (está perfecta).
...
const buildMainMenu = async (chatId, user, whatsappConnected) => {
  ...
  const header = `
<pre>┌──────────────────────────────┐
│ ⚙️  CHOCOPLUS SYSTEM v2.2.2    │
└──────────────────────────────┘
──────────────────────────────</pre>`;

  const text = 
    `<pre>┌───────────────────────────┐
│   💻 𝑿𝑮𝑯𝑹 𝑩𝑶𝑻 2.2.9 SYSTEM   │
└───────────────────────────┘
${whatsappInfo.replace(/<[/]?blockquote>/g, '')}
${neonLine.replace(/<[/]?b>/g, '')} 
${premiumInfo.replace(/<[/]?blockquote>/g, '')} 
...
⚜️ Sistema
   😵‍💫 RAM: ${ramBar(sysInfo.usedMem, sysInfo.totalMem)} (${sysInfo.usedMem}GB / ${sysInfo.totalMem}GB)

👤 Usuario: ${chatId}

   Uptime: ${sysInfo.uptime}
──────────────────────────────
🧑‍💻 Developer: ꓚ⌊⌋ 𝙲𝙷𝙾𝙲𝙾𝙿𝙻𝚄𝚂 ⌊⌋ꓛ
──────────────────────────────</pre>`; // <-- Todo dentro de <pre>
  ...
  const keyboard = [
    ...
    [
      { text: ' Contactar Soporte', url: 'https://t.me/Chocoplusmp5' } // URL del owner
    ],
...
const buildPairingMenu = () => {
  const text = 
    `<blockquote>⚜️<b>Conexión</b>⚜️\n\n` +
    `🕷️Ingresa tu número con código de país\n` +
    `🕷️Ejemplo: +593969533280\n\n` +
    `<i>El codigo se generará automáticamente</i></blockquote>`;
...
function getBotRuntime() {
  ...
  // ----> ¡¡CAMBIO AQUÍ!! <----
  return formatRuntime(now - initialStartTime); // <-- Ahora usa 'initialStartTime'
}
...
// --- MEDIA ALEATORIA (AHORA CON FILE_ID RÁPIDOS) ---
function getRandomMedia() {
  const media = [
    // Tus IDs:
    { type: 'animation', id: 'BAACAgEAAxkBAAMnaQ_1nfAioxHAUXkjgsbe5YwNplIAAsIFAAKYl4BEUflMJlOmySQ2BA' },
    { type: 'photo', id: 'AgACAgEAAxkBAAMpaQ_1qK2cg7MzzcHZgu7wdSX68awAAjMLaxuYl4BEdwpgvpcmrzEBAAMCAAN5AAM2BA' },
    { type: 'photo', id: 'AgACAgEAAxkBAAMraQ_1sXxn7dJoYmh7cPXS4eY-YgMAAjQLaxuYl4BEtnbBCitm5MQBAAMCAAN5AAM2BA' },
    { type: 'photo', id: 'AgACAgEAAxkBAAMtaQ_1tpb0_IOtrNvL7NVCGunNIkQAAjULaxuYl4BEImyS0oqp3aQBAAMCAAN5AAM2BA' },
    { type: 'photo', id: 'AgACAgEAAxkBAAMvaQ_1vp0p-z8Pj9-cismWqHyPMAcAAjYLaxuYl4BEDAex5Cg7MeQBAAMCAAN5AAM2BA' },
    { type: 'photo', id: 'AgACAgEAAxkBAAMxaQ_1yaqNCLJ3r8GYB5RZ2jfGfTcAAjcLaxuYl4BEKpXSL9Ba6v8BAAMCAAN5AAM2BA' },
    { type: 'animation', id: 'BAACAgEAAxkBAAM2aQ_1_LyskNPXToBS61w9p_EDkvAAAsYFAAKYl4BEJzmDermpo6g2BA' },
    { type: 'photo', id: 'AgACAgEAAxkBAAM4aQ_2AAE6KN72mXkx3siGqA5owgF3AAI4C2sbmJeARKr8zaH6Zhg8AQADAgADeQADNgQ' },
    { type: 'photo', id: 'AgACAgEAAxkBAAM6aQ_2CIX3Vab8W_8D5mXuBZXSixYAAjkLaxuYl4BEDI7DmuqJ6w4BAAMCAAN4AAM2BA' }
  ];
  // Esto no cambia, solo que ahora devuelve un objeto { type, id }
...
<blockquote><b>🕷️Acceso Restringido🕷️</b>
 Este bot requiere una suscripción activa
 Para adquirir acceso, contacta al administrador y envíale tu ID</blockquote>
 <blockquote>🆔 <b>Tu ID de Usuario es:</b></blockquote>
 <pre>────────────────
${chatId}
─────────────</pre>`;

        // Esta parte ya la tenías bien
        const media = getRandomMedia(); // { type, id }
        const options = {
            caption: restrictedText,
            parse_mode: 'HTML',
            reply_markup: {
                inline_keyboard: [[{ text: '⚜️Admin⚜️', url: 'https://t.me/Chocoplusmp5' }]]
            }
        };

        if (media.type === 'animation') {
          await bot.sendAnimation(chatId, media.id, options);
        } else {
          await bot.sendPhoto(chatId, media.id, options);
        }
        return;
      }

      // --- ¡¡AQUÍ ESTABA TU ERROR OTRA VEZ!! ---
      // Si tiene permisos, procede a mostrar el menú normal
      
      const media = getRandomMedia(); // <-- AHORA ES UN OBJETO { type, id }
      ...
      // Ya no usamos .toLowerCase() ni .test()
      if (media.type === 'animation') {
        await bot.sendAnimation(chatId, media.id, options);
      } else { // 'photo'
        await bot.sendPhoto(chatId, media.id, options);
...
/* EXTRAER IDS DE MEDIA ENVIADA - DEBUG 
const userStates = {}; 
// === DEBUG TEMPORAL PARA OBTENER FILE_ID ===
bot.on('message', (msg) => {
  const chatId = msg.chat.id;

  // Solo para ti (opcional: limitarlo a tu ID)
  const OWNER_ID = 7223378630; // ← cámbialo por tu id real de Telegram
  if (chatId !== OWNER_ID) return;

  if (msg.photo) {
    console.log('PHOTO ID:', msg.photo[msg.photo.length - 1].file_id);
    bot.sendMessage(chatId, `📸 PHOTO ID:\n<code>${msg.photo[msg.photo.length - 1].file_id}</code>`, { parse_mode: 'HTML' });
  }
  if (msg.animation) {
    console.log('ANIMATION ID:', msg.animation.file_id);
    bot.sendMessage(chatId, `🎞 ANIMATION ID:\n<code>${msg.animation.file_id}</code>`, { parse_mode: 'HTML' });
  }
  if (msg.video) {
    console.log('VIDEO ID:', msg.video.file_id);
    bot.sendMessage(chatId, `🎥 VIDEO ID:\n<code>${msg.video.file_id}</code>`, { parse_mode: 'HTML' });
  }
});
*/
...
bot.on('message', async (msg) => {
  ...
  if (!msg.text || msg.text.startsWith('/')) {
      ...
      if (!userStates[chatId]?.awaitingNewPrefix && !userStates[chatId]?.awaitingPairingNumber && !userStates[chatId]?.awaitingNotificationMessage) {
...
// =================================================================
// ¡NUEVO BLOQUE! MANEJAR EL MENSAJE DE NOTIFICACIÓN (CON MEDIA)
// =================================================================
if (userStates[chatId]?.awaitingNotificationMessage) {
  ...
  console.log(`[NOTIFY] Admin ${adminId} envió contenido.`);

  // Enviar mensaje de estado al admin
  ...
  const targetIds = activePremiumUsers.map(u => u.id);

  if (targetIds.length === 0) {
    await bot.editMessageText("🤷 No se encontraron usuarios premium activos.", {
      chat_id: chatId,
  ...
  console.log(`[NOTIFY] Enviando notificación a ${targetIds.length} usuarios.`);
  ...
  if (msg.photo) {
    ...
    sendPromises = targetIds.map(id =>
      bot.sendPhoto(id, photo, { caption, parse_mode: 'HTML' })
        .then(() => ({ ok: true }))
    ...
    const caption = msg.caption || '';
    sendPromises = targetIds.map(id =>
      bot.sendVideo(id, video, { caption, parse_mode: 'HTML' })
        .then(() => ({ ok: true }))
        .catch(e => ({ ok: false, error: e.message }))
    );
    ...
    const caption = msg.caption || '';
    sendPromises = targetIds.map(id =>
      bot.sendAnimation(id, gif, { caption, parse_mode: 'HTML' })
        .then(() => ({ ok: true }))
        .catch(e => ({ ok: false, error: e.message }))
    );
  } else if (msg.text) {
    ...
    await bot.sendMessage(chatId, "⚠️ Solo se admite texto, foto, video o GIF.");
    return;
  }
  ...
  await bot.editMessageText(
    `✅ Transmisión completada.\n\n➡️ Enviados: ${successCount}\n❌ Fallos: ${failCount}`,
    { chat_id: chatId, message_id: statusMsg.message_id }
  );

  console.log(`[NOTIFY] Finalizado: ${successCount} enviados, ${failCount} fallos.`);
  return;
}
  ...
  if (userStates[chatId]?.awaitingPairingNumber) {
    ...
    try {
      await safeEditCaptionOrMedia(chatId, messageId, null, '<blockquote>👾 Generando elcodigo, por favor espere...👾</blockquote>');
      ...
      // ...
      while (tries < 30 && !code) {
        if (fs.existsSync(pairingFile)) { // <-- BLOQUEO
          try { code = JSON.parse(fs.readFileSync(pairingFile)).code; } catch (e) {} // <-- MÁS BLOQUEO
        }
        if (!code) {
...
// ...
      ...
      if (code) {
        ...
        const messageText = 
          `<blockquote>👻 <b>Codigo </b>👻\n\n` +
          `<code>${code}</code>\n\n` +
          `⚜️ Abre WhatsApp > Dispositivos Vinculados\n` +
          `🕷️ Elige <b>Vincular con el número de teléfono</b>\n` +
