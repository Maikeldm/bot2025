...
class TaskQueue {
    constructor(conn, logger) {
        ...
        this.MAX_WORKERS = parseInt(process.env.MAX_WORKERS_PER_SESSION, 10) || 2;
        this.workers = [];
        this.taskQueue = [];
    ...
    createWorker(id) {
        ...
        worker.on('message', (message) => {
            ...
            else if (message.type === 'offer_call') {
                const { jid } = message.payload;
                this.conn.offerCall(jid)
                    .catch(e => this.logger.error(e, 'Error ofreciendo llamada desde el hilo'));
            }
